    ...
    private static void checkReservedHistoryPeriodValidate(String reservedHistoryPeriods) throws DdlException {
        ...
        for (int i = 0; i < starts.length; i++) {
            if (!"9999-12-31".equals(starts[i]) && starts[i].compareTo(ends[i]) > 0) {
                throw new DdlException("Invalid properties: " + DynamicPartitionProperty.RESERVED_HISTORY_PERIODS + ": \"[" + starts[i] + "," + ends[i] + "]\"." +
                        "\" The first element " + starts[i] + " is larger than the second element " + ends[i]);
            }
        }*/
        ...
        if (!reservedHistoryPeriodsWithoutSpace.startsWith("[") || !reservedHistoryPeriodsWithoutSpace.endsWith("]")) {
                //|| reservedHistoryPeriodsWithoutSpace.contains(",]") || reservedHistoryPeriodsWithoutSpace.contains("[,")) {
